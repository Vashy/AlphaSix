...
\section{Processi di supporto}\label{PS}
	...
	\subsection{Documentazione}\label{PS:Documentazione}
		...
		\subsubsection{Implementazione}\label{PS:Documentazione:Implementazione}
			...
			Prima di iniziare a redigere i documenti, è stato creato un template per \LaTeX \ (\S\ref{LaTeX})
	...
	\subsection{Verifica}
		...
		\subsubsection{Scopo}
		Questa sezione vuole descrivere come il team di sviluppo esegue la fase di verifica, per capire se i prodotti e i processi sono conformi a quanto ci si attende. Se ciò accede il team precederà alla fase successiva del progetto, in caso contrario sarà necessario il ritorno ad una fase stabile del progetto e da lì ripartire prendendo in considerazione i risultati della precedente verifica. 

		%\subsubsection{Aspettative} %da mettere nel PdQ

		\subsubsection{Descrizione}
		Ogni processo e prodotto deve essere valutato in modo quantificabile inserendoli in delle metriche apposite quando possibile e stabilendo il risultato che si vuole raggiungere.
		
		Come indicato dal \textit{Ciclo di Deming} nel \Doc{\PdQ} alla \textit{Appendice A}, nel momento in cui tale risultato sarà raggiunto, se esso non è il migliore, servirà come "base" per alzare il livello di qualità di quel processo o prodotto.

		\subsubsection{Metriche}
		La denominazione delle metriche è già stata descritta in §3.1.3.3, qui vengono descritte.
