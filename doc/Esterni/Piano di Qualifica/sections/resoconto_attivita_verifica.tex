...
I test che vengono introdotti nel corso del progetto non vanno a sostituire i test precedentemente sviluppati. Ogni test, appena è creato viene eseguito periodicamente fino alla fine del progetto, per evitare che i nuovi cambiamenti possano reintrodurre errori precedentemente risolti.
   ...
   \end{itemize}
	...
	\subsection{Quarto periodo (RA)}\label{QuartoPeridodoRA}
    ...
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.9\textwidth]{img/cruscotti/RA/MPD001.png}
        \label{immaginegulpeaseRA}
        \caption{Diagramma con valori misurati tramite MPD001 Indice di Gulpease}
    \end{figure}
    ...
    \begin{itemize}
        ...
        \item \textbf{Valutazione}: soddisfacente.
        \item \textbf{Considerazioni}: ogni documento ha degli alti e bassi nel corso della stesura, ma infine tutti rispettano i valori desiderati. Studiando il diagramma si nota che, a differenza del
        precedente periodo di \RQ, i valori ottenuti dalle verifiche non variano di molto durante la
        stesura. Questo è una conseguenza del fatto che i documenti non hanno subito radicali modifiche.
    ...
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.9\textwidth]{img/cruscotti/RA/MPD002.png}
        \label{immagineCorrettezzaOrtograficaRA}
        \caption{Diagramma con valori misurati tramite MPD002 Correttezza ortografica}
    \end{figure}
    ...
    \begin{itemize}
        ...
        \item \textbf{Descrizione}: per ogni documento è mostrato l'andamento del numero di errori ortografici e la soglia (in nero) messa a zero.
        \item \textbf{Valutazione}: soddisfacente.
        \item \textbf{Considerazioni}: studiando il grafico si nota che i documenti con più errori sono i manuali, essendo documenti ancora poco
        maturi però proseguendo con le varie verifiche notiamo una costante diminuzione degli errori grammaticali.
    \end{itemize}
    ...
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.9\textwidth]{img/cruscotti/RA/MPR001.png}
        \label{immagineVarianzaPianificazioneRA}
        \caption{Diagramma con valori misurati tramite MPR001 Varianza della pianificazione}
    \end{figure}
    ...
    \begin{itemize}
        ...
        \item \textbf{Valutazione}: soddisfacente.
        \item \textbf{Considerazioni}: in questo periodo abbiamo dovuto ridurre le ore al
        \Res\ per darne al \Ver\ e al \Progr, avendo bisogno di alcune ore per fare delle piccole correzioni
        al codice.
    \end{itemize}
    ...
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.9\textwidth]{img/cruscotti/RA/MPR002.png}
        \label{immagineVarianzaCostiRA}
        \caption{Diagramma con valori misurati tramite MPR002 Varianza dei costi}
    \end{figure}
    ...
    \begin{itemize}
        ...
        \item \textbf{Descrizione}: vengono mostrate le variazioni della spesa attribuita ad ogni ruolo, inoltre ne viene mostrato il totale attraverso delle colonne (in viola) coi relativi valori.
        \item \textbf{Valutazione}: soddisfacente.
        \item \textbf{Considerazioni}: la diminuzione delle ore assegnate al \Res\ a favore di quelle per il \Ver\ e per il \Progr\, non ha portato nessun cambiamento del preventivo. Un dato che
        risulta essere soddisfacente.
    \end{itemize}
    ...
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.9\textwidth]{img/cruscotti/RA/MPR003(1).png}
        \label{immagineAderenzaStandard1RA}
        \caption{Diagramma con valori misurati tramite MPR003 Aderenza agli standard (1)}
    \end{figure}

    \begin{figure}[H]
        \centering
        \includegraphics[width=0.9\textwidth]{img/cruscotti/RA/MPR003(2).png}
        \label{immagineAderenzaStandard2RA}
        \caption{Diagramma con valori misurati tramite MPR003 Aderenza agli standard (2)}
    \end{figure}

    \begin{figure}[H]
        \centering
        \includegraphics[width=0.9\textwidth]{img/cruscotti/RA/MPR003(3).png}
        \label{immagineAderenzaStandard3RA}
        \caption{Diagramma con valori misurati tramite MPR003 Aderenza agli standard (3)}
    \end{figure}
    ...
    \begin{itemize}
        ...
        \item \textbf{Descrizione}: per ogni attributo dei processi viene mostrato in che percentuale sono soddisfatti attraverso le colonne (in blu), mostrando anche la soglia minima e massima (in grigio e nero) che questi valori dovrebbero avere.
        \item \textbf{Valutazione}: soddisfacente.
        \item \textbf{Considerazioni}: il miglioramento per quanto riguarda l'aderenza agli standard, dopo un duro lavoro, è finalmente sufficiente per gli obiettivi prefissati.
    \end{itemize}
    ...
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.9\textwidth]{img/cruscotti/RA/MPR004.png}
        \label{immagineFrequenzaCommitRA}
        \caption{Diagramma con valori misurati tramite MPR004 Frequenza di commit nella repository}
    \end{figure}
    ...
    \begin{itemize}
        ...
        \item \textbf{Descrizione}: è mostrato il numero di commit effettuati tra una verifica e la successiva.
        \item \textbf{Valutazione}: soddisfacente.
        \item \textbf{Considerazioni}: studiando il diagramma notiamo che il numero di commit nel primo periodo è calato, sintomo dei vari colloqui affrontati
        dai membri del gruppo. Dal secondo periodo il gruppo ha ripreso a lavorare con impegno e costanza.
    \end{itemize}
    ...
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.9\textwidth]{img/cruscotti/RA/MPR010.png}
        \label{immagineModuliCodificatiRA}
        \caption{Diagramma con valori misurati tramite MPR010 Moduli codificati prima della progettazione}
    \end{figure}
    ...
    \begin{itemize}
        ...
        \item \textbf{Descrizione}: è mostrato il numero di moduli codificati prima dell'effettiva progettazione.
        \item \textbf{Valutazione}: soddisfacente.
        \item \textbf{Considerazioni}: i valori sono rimasti invariati perché la fase di progettazione è terminata lo scorso periodo. Inoltre non sono stati codificati ulteriori moduli non previsti dalla progettazione.
    \end{itemize}
    ...
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.9\textwidth]{img/cruscotti/RA/MPR011.png}
        \label{immagineNumDesignPatternRA}
        \caption{Diagramma con valori misurati tramite MPR011 Numero di design pattern applicati}
    \end{figure}
    ...
    \begin{itemize}
        ...
        \item \textbf{Descrizione}: è mostrato il numero di design pattern applicati tra una verifica e la successiva.
        \item \textbf{Valutazione}: soddisfacente.
        \item \textbf{Considerazioni}: i valori sono rimasti invariati perché la fase di progettazione è terminata lo scorso periodo e non sono state
        implementate modifiche strutturali.
    \end{itemize}
    ...
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.9\textwidth]{img/cruscotti/RA/MPR012.png}
        \label{immagineModuliNonTestatiRA}
        \caption{Diagramma con valori misurati tramite MPR012 Moduli non testati}
    \end{figure}
    ...
    \begin{itemize}
        ...
        \item \textbf{Descrizione}: viene mostrato il numero di moduli non testati.
        \item \textbf{Valutazione}: soddisfacente.
        \item \textbf{Considerazioni}: come ci eravamo promessi nel periodo di \RP\ siamo riusciti a testare tutti i moduli raggiungendo il valore desiderato.
    \end{itemize}

    \subparagraph{MPR013 Percentuale metriche rispettate}

    \begin{figure}[H]
        \centering
        \includegraphics[width=0.9\textwidth]{img/cruscotti/RA/MPR013.png}
        \label{immaginePercentualeMetricheRA}
        \caption{Diagramma con valori misurati tramite MPR013 Percentuale metriche rispettate}
    \end{figure}

    \begin{itemize}
        \item \textbf{Obiettivo}: QPR016 Possedere un prodotto sviluppato correttamente.
        \item \textbf{Valore desiderato}: 60\%.
        \item \textbf{Descrizione}: viene mostrata la percentuale delle metriche rispettate e non rispettate nel corso della \RP, \RQ\ e \RA\ nelle
        loro fasi di verifica.
        \item \textbf{Valutazione}: soddisfacente.
        \item \textbf{Considerazioni}: come ci aspettavamo il diagramma non ci restituisce un valore alto, questo è dovuto al fatto che analizza i risultati
        dei primi periodi quando non eravamo in grado di soddisfare sempre i valori desiderati. Questo diagramma ha lo scopo di fare un vero e proprio resoconto
    ...
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.9\textwidth]{img/cruscotti/RA/MPS001.png}
        \label{immaginePresenzaBugRA}
        \caption{Diagramma con valori misurati tramite MPS001 Presenza di bug}
    \end{figure}
    ...
    \begin{itemize}
        ...
        \item \textbf{Descrizione}: viene mostrato il numero totale di bug rilevati da SonarQube.
        \item \textbf{Valutazione}: soddisfacente.
        \item \textbf{Considerazioni}: anche dopo la conclusione quasi definitiva del prodotto software, abbiamo riscontrato alcuni bug dovuti anche alle ultime funzionalità marginali che abbiamo aggiunto.
    \end{itemize}
    ...
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.9\textwidth]{img/cruscotti/RA/MPS002.png}
        \label{immaginePresenzaVulnerabilitàRA}
        \caption{Diagramma con valori misurati tramite MPS002 Presenza di vulnerabilità}
    \end{figure}
    ...
    \begin{itemize}
        ...
        \item \textbf{Descrizione}: viene mostrato il numero totale di vulnerabilità rilevate da SonarQube.
        \item \textbf{Valutazione}: soddisfacente.
        \item \textbf{Considerazioni}: durante le fasi di test si presentavano temporanee vulnerabilità nel codice.
    \end{itemize}
    ...
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.9\textwidth]{img/cruscotti/RA/MPS003.png}
        \label{immaginePresenzaCodeSmellRA}
        \caption{Diagramma con valori misurati tramite MPS003 Presenza di code smell}
    \end{figure}
    ...
    \begin{itemize}
        ...
        \item \textbf{Descrizione}: viene mostrato il numero totale di code smell rilevati da SonarQube.
        \item \textbf{Valutazione}: poco soddisfacente.
        \item \textbf{Considerazioni}: come indicato alla revisione precedente, ci siamo concentrati sul sistemare le ultime sregolatezze del codice, tra cui anche i code smell.
    \end{itemize}
    ...
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.9\textwidth]{img/cruscotti/RA/MPS004.png}
        \label{immaginePresenzaDupplicazioneCodiceRA}
        \caption{Diagramma con valori misurati tramite MPS004 Duplicazione del codice}
    \end{figure}
    ...
    \begin{itemize}
        ...
        \item \textbf{Descrizione}: viene mostrata la percentuale di codice duplicato rilevato da SonarQube.
        \item \textbf{Valutazione}: soddisfacente.
        \item \textbf{Considerazioni}: alla termine del progetto non siamo riusciti ad azzerare la duplicazione di codice, anche se non era uno dei nostri obiettivi principi per la consegna dei prodotti.
    \end{itemize}
    ...
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.9\textwidth]{img/cruscotti/RA/MPS017.png}
        \label{immagineCodeCoverageRA}
        \caption{Diagramma con valori misurati tramite MPS017 Code coverage}
    \end{figure}
    ...
    \begin{itemize}
        ...
        \item \textbf{Descrizione}: viene mostrata la percentuale di codice testato rilevato da SonarQube.
        \item \textbf{Valutazione}: soddisfacente.
        \item \textbf{Considerazioni}: all'ultima verifica il risultato ha raggiunto la soglia minima, purtroppo senza toccare la soglia massima. In particolare abbiamo avuto difficoltà a verificare la parte del progetto scritta in \gloss{JavaScript}.
    \end{itemize}

	\subparagraph{MPS018 Test di sistema}

	\begin{figure}[H]
		\centering
		\includegraphics[width=0.9\textwidth]{img/cruscotti/RA/MPS018.png}
		\label{immagineTestsistemaRA}
		\caption{Diagramma con valori misurati tramite MPS018 Test di sistema}
	\end{figure}

	\begin{itemize}
		\item \textbf{Obiettivo}: QPS007 Superamento test di sistema.
		\item \textbf{Valore desiderato}: 100\%.
		\item \textbf{Descrizione}: viene mostrata la percentuale di codice coperto da test di sistema.
		\item \textbf{Valutazione}: soddisfacente.
		\item \textbf{Considerazioni}: fin da subito non tutti i test di sistema non sono stati superati, dato che \DZ\ di \II\ ci ha chiesto, durante un colloquio, di modificare alcune pagine del Gestore Personale.
	\end{itemize}
    ...
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.9\textwidth]{img/cruscotti/RA/MPS019.png}
        \label{immagineTestIntegrazioneRA}
        \caption{Diagramma con valori misurati tramite MPS019 Test di integrazione}
    \end{figure}
    ...
    \begin{itemize}
        ...
        \item \textbf{Descrizione}: viene mostrata la percentuale di codice coperto da test di integrazione.
        \item \textbf{Valutazione}: soddisfacente.
        \item \textbf{Considerazioni}: provando ad aggiungere nuove funzionalità si sono aggiunti ulteriori test di sistema che non sono stati superati fin da subito.
    \end{itemize}
    ...
    \begin{figure}[H]
        \centering
        \includegraphics[width=0.9\textwidth]{img/cruscotti/RA/MPS020.png}
        \label{immagineTestunitaRA}
        \caption{Diagramma con valori misurati tramite MPS020 Test di unità}
    \end{figure}
    ...
    \begin{itemize}
        ...
        \item \textbf{Descrizione}: viene mostrata la percentuale di codice coperto da test di unità.
        \item \textbf{Valutazione}: soddisfacente.
        \item \textbf{Considerazioni}: come detto per i test di integrazione, con l'introduzione delle ultime funzionalità sono stati introdotti nuovi test che non venivano superati subito.
    \end{itemize}
