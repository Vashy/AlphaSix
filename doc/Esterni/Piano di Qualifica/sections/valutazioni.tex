...
\section{Valutazioni per il miglioramento}	\label{valutazioni per il miglioramento}
	...
	\subsubsection{Valutazione dei ruoli}
		...
		\paragraph{Analista}
			\begin{itemize}
				\item \textbf{Problema riscontrato}: la principale difficoltà è stata la stesura dell'\AdRd\ in quanto i contenuti di questo documento sono nuovi e per noi di difficile comprensione. In particolar modo l'individuazione dei corretti \gloss{casi d'uso} del progetto.
	...
	\subsubsection{Integrità di prodotti e strumenti}
		\begin{itemize}
			...
			\item \textbf{Soluzione proposta}: prima di effettuare una modifica nella repository è tassativo controllare che non si presentino problemi a qualcuno di noi. Dunque, oltre a dover avere ben chiaro il contenuto delle \NdPd, è necessario avvisare tempestivamente chi ha introdotto l'errore nella repository oppure nel prodotto testato o utilizzato in fase di sviluppo. La maggior parte di questi errori dovrebbero essere segnalati dal \Ver, ma è possibile anche che le segnalazioni arrivino da chi ricopre altri ruoli.
		\end{itemize}
	...
	\subsection{Quarto periodo (RA)}\label{valutazioni per il miglioramento:RA}
	...
	In conclusione è stato utile tracciare i risultati delle varie metriche per avere una visione generale, ma anche specifica, dell'andamento del progetto. Ancora di più l'auto valutazione dei vari membri del gruppo attraverso suggerimenti reciproci e aiuti che hanno ci aiutato a restare un gruppo unito e con lo stesso livello di conoscenze.
