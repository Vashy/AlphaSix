...
La view è pertanto totalmente passiva e l’unica cosa che fa è inoltrare i comandi ricevuti dall’utente all’\gloss{API REST}. \par
...
\gloss{MongoDB} è un database non relazionale e orientato ai documenti (tabelle). Ci è molto utile in questo progetto perché ci dà la possibilità di aggiungere dati ai documenti senza modificarne la struttura.
...
\end{figure}
    ...
    \paragraph{Diagramma delle classi}
    Il Gestore Personale (ClientGP) si interfaccia con Kafka attraverso KafkaProducer e KafkaConsumer, e per elaborare i messaggi che riceve crea un'implementazione di Processor. Questa, in base alle informazioni nel messaggio, indica qual è il destinatario del messaggio e attraverso quale applicazione notificarlo, interrogando il database con MongoFacade.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{img/Consumer-MsgTelegram.png}\\
    \caption[Diagramma di sequenza TelegramConsumer]{Diagramma di sequenza dell'invio di un messaggio da parte di TelegramConsumer}
    % \label{fig:GP-Kafka}
\end{figure}
...
Nel seguente diagramma di sequenza è possibile vedere il flusso di esecuzione che parte dalla ricezione del messaggio dalla coda \texttt{email}
di Kafka all'invio del messaggio all'utente finale tramite server SMTP.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{img/Consumer-MsgEmail.png}\\
    \caption[Diagramma di sequenza EmailConsumer]{Diagramma di sequenza dell'invio di un messaggio da parte di EmailConsumer}
    % \label{fig:GP-Kafka}
\end{figure}
