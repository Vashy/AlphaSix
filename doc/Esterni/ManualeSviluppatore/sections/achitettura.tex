...
\begin{itemize}
    ...
    \item \texttt{KafkaProducer}, classe concreta del package di libreria \texttt{kafka}. Ha il compito di interfacciarsi con il broker Kafka e di immettere
        in esso i messaggi. È costruito su di esso un \textit{adapter}, in cui KafkaProducer viene adattato all'astrazione \texttt{Producer}
    \item Package \texttt{webhook}, che ha il compito di effettuare il parsing dei messaggi in entrata
\end{itemize}
...
Abbiamo implementato per GitlabProducer 4 tipologie di Webhook.
