...
e di immettere nella \textit{coda redmine} di Kafka i messaggi.
...
Per Redmine, è presente una sola tipologia di webhook. Tenendo a mente la \gloss{open-closed principle},
...
funzionalità che sono identiche al Producer di Redmine. \par Per tenere isolate
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{img/Client_attivita.png}\\
...
Nel seguente diagramma di sequenza è possibile vedere il flusso di esecuzione che parte dalla ricezione del messaggio dalla \textit{coda telegram}
...
\begin{itemize}
    \item \texttt{KafkaConsumer} dalla libreria esterna \texttt{kafka-python}: offre le funzionalità di ascolto
        di una o più code specifiche. È costruito su di esso un adapter, per adattare \textit{KafkaConsumer} alle funzionalità
        di \texttt{Consumer}.
...
\begin{itemize}
    \item \texttt{KafkaConsumer} della libreria esterna \texttt{kafka-python}: offre le funzionalità di ascolto dei messaggi
        proveniente da una o più code specifiche. È costruito su di esso un adapter, per adattare le funzionalità di \textit{KafkaConsumer} a
        \texttt{Consumer}.
