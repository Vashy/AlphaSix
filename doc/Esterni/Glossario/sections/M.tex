\lettera{M}\label{M}
    ...
    \parola{Mock}{%
        Oggetto usato all'interno di un'ambiente di testing il quale simula un altro oggetto non presente all'interno del test stesso.
    }\label{Mock}
    ...
    \parola{Modello incrementale}{%
        Si tratta di un modello di sviluppo che prevede l'iterazione di alcuni passaggi che portano il prodotto ad essere sviluppato attraverso un serie di incrementi di portata sempre equivalente. Le sue fasi principali sono:
        \begin{itemize}
            \item Analisi preliminare
            \item Consolidamento analisi e progettazione
            \item Realizzazione
        \end{itemize}
        Le ultime due fasi continuano a ripetersi in modo alternato per effettuare l'incremento.
    }\label{Modello incrementale}

    \parola{Modulo}{%
        Aggregato che eseguito singolarmente non riesce a restituire output significativi. È un insieme di unità software.
    }\label{Modulo}
