...
#1 & #2 & #3 \\
	...
	\newcommand{\addQNumber}[0]{\decrQ\thevaQ} % Comando per generare il valore incrementato di uno rispetto a prima
	
	\newcounter{secondQ} % per il secondo livello del requisito
	\addtocounter{secondQ}{1}
	\newcommand{\secdecrQ}{\addtocounter{secondQ}{+1}} % Comando per l'aumento automatico del counter per il secondo livello
	\newcommand{\addSecQNumber}[0]{\secdecrQ\thesecondQ} % Comando per generare il valore incrementato di uno rispetto a prima
	\newcommand{\resetQCounter}{\addtocounter{secondQ}{-\thesecondQ}\addtocounter{secondQ}{+1}}
	\newcommand{\decSecQ}[0]{\resetQCounter\thesecondQ}
	...
	\newcommand{\addVNumber}[0]{\decrV\thevaV} % Comando per generare
	\addtocounter{vaV}{1}
	...
	\begin{table}[H]
		\begin{paddedtablex}[1.7]{\textwidth}{cXc} %\rowcolors{1}{\tablegray}{\lightgray}
			...
			\req{R\thevaV.1V0}{È possibile avere un terzo componente applicativo Producer oltre ai due obbligatori}{Capitolato}
			\Vreq{2}{Devono essere sviluppati due componenti applicativi Consumer tra Telegram, Email e Slack}{Capitolato}
			\req{R\thevaV.1V0}{È possibile avere un terzo componente applicativo Consumer oltre ai due obbligatori}{Capitolato}
			\Vreq{2}{Docker deve essere la tecnologia di riferimento per l'istanziazione di tutte le componenti}{Capitolato}
			\req{R\thevaV.1V2}{È necessario presentare un Dockerfile per ogni componente}{Capitolato}
